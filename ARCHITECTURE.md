...
sequenceDiagram
    ...
    VM->>VM: acquire Mutex "GooglePhotosUploader-SingleRun"
    ...
    VM->>VM: append RunHistoryEntry, release Mutex
...
### 7.3 The single-run mutex

Both `DashboardViewModel.RunNowAsync` and `Program.RunHeadlessAsync` take
the exact same named `Mutex` (`GooglePhotosUploader-SingleRun`, defined once
as `DashboardViewModel.SingleRunMutexName`) with `WaitOne(TimeSpan.Zero)`
(non-blocking: if already held, give up immediately instead of queuing).
This prevents a manual run and a scheduled run from racing on the same
`state.json`. If you add a **third** way to trigger an upload, it must
acquire this same mutex the same way.
